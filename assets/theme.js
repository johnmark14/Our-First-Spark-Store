...
slate.Variants = (function() {
  ...
  Variants.prototype = _.assignIn({}, Variants.prototype, {
    ...
    _onSelectChange: function() {
      ...
      // MOd by PVA39 John Mark
      const regularPrice = theme.Currency.formatMoney(this.currentVariant.compare_at_price, theme.moneyFormat);
      const salePrice = theme.Currency.formatMoney(this.currentVariant.price, theme.moneyFormat);
      const rp = regularPrice.substring(1, regularPrice.length);
      const sp = salePrice.substring(1, salePrice.length);

      const save = Math.round(Number((rp-sp)/rp * 100.0));

      document.getElementById('discount-label').innerText = `Save ${save}%`;
...
theme.Cart = (function() {
  var selectors = {
    ...
    cartItemImageContainer: '[data-cart-item-image-container]',
    cartItemImageIcon: '[data-cart-item-image-icon]',
    ...
    _createLineItemList: function(state) {
      return $.map(
        ...
        function(item, index) {
          ...
          this._setLineItemImage($item, item.featured_image, item.properties);

          this._setLineItemImageIcon($item, item.properties);
...
// /////////////////// Cart Image Properties Mod by PV39: John Mark//////////////////////////////////////////////////
    _setLineItemImage: function($item, featuredImage, properties) {
      ...
      propertiesArray.forEach(function(property) {
        if(property[0] == '_Edited Photo') imageSRC.photo = property[1];
        if(property[0] == '_Engraving Photo') imageSRC.engraving = property[1];
      }); 
      
      var cartImageSRC;
      if(imageSRC.photo) {
        cartImageSRC = imageSRC.photo;
      } else if(imageSRC.engraving) {
        cartImageSRC = imageSRC.engraving;
      }
      
      var imageURL;
      if(cartImageSRC) { 
        imageURL = cartImageSRC;
        $image.attr('id', 'cart-image');
      } else {
      ...
      if (imageURL) {
        $image
          ...
          .attr('src', imageURL)
          .attr('style', 'object-fit: scale-down;') 
          .attr('data-engraving-photo', imageSRC.engraving)
          .removeClass(classes.hide);
      } else {
        $image.remove();
      }
    },
// /////////////////// Cart Image Icon for preview Properties Mod by PV39: John Mark//////////////////////////////////////////////////
    _setLineItemImageIcon: function($item, properties) {
      var $imageContainer = $(selectors.cartItemImageContainer, $item);
      var $imageIcon = $(selectors.cartItemImageIcon, $item);
      ...
      if(cartImageSRC) {
        $imageContainer.addClass('cart-onclick display-flex justify-content-c');
        
        $imageIcon
        .addClass('fa fa-search-plus') 
        .attr('style', 'font-size: 18px; padding: 5px;')
        .removeClass(classes.hide);
      } else {
        $imageContainer.removeClass('cart-onclick display-flex justify-content-c');
        $imageIcon
        .removeClass('fa fa-search-plus')
        .addClass(classes.hide);
      }
    },
...
// /////////////////// Cart Properties Mod by PV39: John Mark//////////////////////////////////////////////////
    _getPropertyList: function(properties) {
      ...
      return $.map(
        ...
        function(property) {
          ...
          // Mod by PVA39 John Mark: This is used to capture the correct content when cart Ajax is tru
          if(property[0] == 'Original Photo') {
            $propertyElement
              .find(selectors.cartItemPropertyName)
              .text('Photo:');
          } else if(property[0] == '_Edited Photo') {
            return; 
          } else {
            $propertyElement
              .find(selectors.cartItemPropertyName)
...
//Product thumbnails as slider
    ...
    _addItemToCart: function($form) {
      ...
      var params = {
        ...
        success: function(item) {
          if($('#chk-giftbox').is(":checked") && $('#chk-vip-service').is(':checked')) {
            $.ajax({
              type: "POST",
              url: '/cart/add.js',
              data: { items: [
                {
                quantity: 1,
                id: $('#chk-giftbox').attr("data-prod-id")
                },
                {
                  quantity: 1,
                  id: $('#chk-vip-service').attr("data-prod-id")
                }
            ]},
              datatype: 'json',
              complete: function() {
          ...
          } else if($('#chk-giftbox').is(":checked")) {
            $.ajax({
              type: "POST",
              url: '/cart/add.js',
              data: { items: [{
          ...
          } else if ($('#chk-vip-service').is(':checked')) {
            $.ajax({
              type: "POST",
              url: '/cart/add.js',
              data: { items: [{
    ...
    _updateCartPopupContent: function(item) {
      ...
      this._setCartPopupImage(item.featured_image.url, item.featured_image.alt, item.properties);
...
// /////////////////// Popup Image Properties Mod by PV39: John Mark//////////////////////////////////////////////////
    _setCartPopupImage: function(imageUrl, imageAlt, properties) {
      if (imageUrl === null) return;

      this.$cartPopupImageWrapper.removeClass(this.classes.hidden);
      var sizedImageUrl = theme.Images.getSizedImageUrl(imageUrl, '200x');
      var image = document.createElement('img');
      var propertiesArray = properties ? Object.entries(properties) : '';
      var imageSRC = {};
      propertiesArray.forEach(function(property) {
        if(property[0] == '_Edited Photo') imageSRC.photo = property[1];
        if(property[0] == '_Engraving Photo') imageSRC.engraving = property[1];
...
// /////////////////// Popup Properties Mod by PV39: John Mark//////////////////////////////////////////////////
    _getPropertyList: function(properties) {
      ...
      propertiesArray.forEach(function(property) {
        ...
        if (property[0] == '_Edited Photo') {
            return;
          } else if(property[0] == 'Original Photo') {
            return;
          } else {
            propertyListHTML =
              propertyListHTML +
              '<li class="product-details__item product-details__item--property">' +
              '<span class="product-details__property-label">' + property[0] +
              ': </span>' + property[1];
              ': ' + '</li>';
...
//STICKY HEADER WITHOUT THE ANNOUNCEMENT BAR
function headerSize() {
